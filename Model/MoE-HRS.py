...
class MoEModel_CNN(nn.Module):
    def __init__(self, num_experts=3, num_snps=50):
        super(MoEModel_CNN, self).__init__()
        self.experts = nn.ModuleList([TransformerExpert(num_snps, model_dim=64, num_heads=4, num_layers=2) for _ in range(num_experts)])
        # self.gate = nn.Linear(num_snps * 100, num_experts)  # Gate based on input SNP data (flattened)
        self.gate = GatingNetwork(num_snps, num_experts)
    ...
    def forward(self, x):
        ...
        x = self.relu(self.fc1(x))
        x = torch.cat((x, weighted_sum), -1)
        x = self.fc2(x)

        return x

class CNN(nn.Module):
    def __init__(self, num_snps=50):
...
class MoEModel_TransCNN(nn.Module):
    def __init__(self, num_experts_tran=3, num_experts_cnn=1, num_snps=50):
        super(MoEModel_TransCNN, self).__init__()
        self.experts = nn.ModuleList([TransformerExpert(num_snps, model_dim=128, num_heads=4, num_layers=2) for _ in range(num_experts_tran)])
        # self.gate = nn.Linear(num_snps * 100, num_experts)  # Gate based on input SNP data (flattened)
        self.gate = GatingNetwork(num_snps, num_experts_tran)
        self.gate2 = GatingNetwork(num_snps, num_experts_cnn)

        self.experts2 = nn.ModuleList([CNN(num_snps) for _ in range(num_experts_cnn)])
        self.softmax = nn.Softmax()
        self.fc_out = nn.Linear(num_experts_tran+num_experts_cnn, 1)
    ...
    def forward(self, x):
        ...
        # print(f"expert_outputs: {expert_outputs}; gate_weights.unsqueeze(2) * expert_outputs: {gate_weights.unsqueeze(2) * expert_outputs}")
        # weighted_sum = torch.sum(gate_weights.unsqueeze(2) * expert_outputs, dim=1)  # Weighted sum of expert outputs
        # weighted_sum = weighted_sum.squeeze(1)
        weighted_exp1 = gate_weights.unsqueeze(2) * expert_outputs
        # print(f"weighted_exp1 111 shape: {weighted_exp1.shape}")
        weighted_exp1 = weighted_exp1.squeeze(-1)
        # print(f"weighted_exp1 222 shape: {weighted_exp1.shape}")

        gate_weights2 = F.softmax(self.gate2(x_flat), dim=1)
        # print(f"gate_weights2 shape: {gate_weights2.shape}")
        expert2_outputs = torch.stack([expert(x) for expert in self.experts2], dim=1)
        # print(f"expert2_outputs shape: {expert2_outputs.shape}")
        # print(f"expert2_outputs: {expert2_outputs}; gate_weights2.unsqueeze(2) * expert2_outputs: {gate_weights2.unsqueeze(2) * expert2_outputs}")
        # weighted_sum2 = torch.sum(gate_weights2.unsqueeze(2) * expert2_outputs, dim=1)
        # # weighted_sum2 = weighted_sum2.squeeze(1)
        # output = torch.cat((weighted_sum, weighted_sum2), -1)
        # print(f"weighted_sum: {weighted_sum.shape}; gate_weights2: {weighted_sum2.shape}; output: {output.shape}")
        weighted_exp2 = gate_weights2.unsqueeze(2) * expert2_outputs
        # print(f"weighted_exp2 111 shape: {weighted_exp2.shape}")
        weighted_exp2 = weighted_exp2.squeeze(-1)
        # print(f"weighted_exp2 222 shape: {weighted_exp2.shape}")
        output = torch.cat((weighted_exp1, weighted_exp2), -1)
        # print(f"output shape: {output.shape}")
        score = self.fc_out(output)
        return score
...
def train_model(model, train_loader, criterion, optimizer, num_epochs, test_loader):
    for epoch in range(num_epochs):
        ...
        for snps, risk_scores in train_loader:
            ...
            scores = model(snps)
            # print(f"Predicted scores shape: {scores.shape}; target risk_scores shape: {risk_scores.unsqueeze(1)}")
            print(f"Predicted scores: {scores}; target risk_scores: {risk_scores}")
        ...
        for metric, value in metrics.items():
            print(f'Lucas Epoch: {epoch}; Evaluation result: {metric}: {value:.4f}')
...
def evaluate_model(model, test_loader, threshold=0.5):
    ...
    with torch.no_grad():
        for snps, risk_scores in test_loader:
            ...
            scores = model(snps)
            if type(scores.squeeze().tolist()) == float:
                all_preds.append(scores.squeeze().tolist())
            else:
...
def main(
        ...
        output_size: int = 1,
        num_experts_trans: int = 3,
        num_experts_cnn: int = 1,
        num_epochs: int = 100,
        batch_size: int = 512,
    ...
    print(
        ...
        f"num_experts_trans: {num_experts_trans}\n"
        f"num_experts_cnn: {num_experts_cnn}\n"
    ...
    # model = MoEModel3(num_experts, input_size).to(gpu_device)
    # model = MoEModel_CNN(num_experts, input_size).to(gpu_device)
    model = MoEModel_TransCNN(num_experts_trans, num_experts_cnn, input_size).to(gpu_device)
