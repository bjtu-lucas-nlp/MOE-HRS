...
class LSTMModel(nn.Module):
    ...
    def forward(self, x):
        h0 = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(gpu_device)
        c0 = torch.zeros(self.num_layers, x.size(0), self.hidden_size).to(gpu_device)
...
def main(
    ...
    model = LSTMModel(input_size, hidden_size, num_lstm_layers)
